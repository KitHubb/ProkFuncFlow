...
def main():
 ...
 for k in ("analysis-config","run-id","outdir","project-dir","checkm2-container","gtdbtk-container","drep-container","checkm2-db","gtdbtk-db","environment","containers","versions","checkpoint"): p.add_argument("--"+k,required=True)
 a=p.parse_args()
 for x in (a.analysis_config,a.checkm2_container,a.gtdbtk_container,a.drep_container,a.checkm2_db):
 ...
 if "1.0.2" not in versions["checkm2"] or "2.6.1" not in versions["gtdbtk"] or versions["drep"] != "3.5.0" or "1.33" not in versions["fastani"]: raise ValueError("pinned version mismatch")
 git={}
 ...
 images=[("checkm2",a.checkm2_container,sha(a.checkm2_container)),("gtdbtk",a.gtdbtk_container,sha(a.gtdbtk_container)),("drep",a.drep_container,sha(a.drep_container))]
